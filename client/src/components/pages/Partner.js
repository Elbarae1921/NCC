...
export class Partner extends Component {
  // set the state
  state = {
    // these properties are used to switch between registration mode and login mode
    result: "",
    error: "",
    login: true,
    switch: "registration",
  ...
  changeMode = () => {
    ...
    this.setState({
      ...
      error: this.state.error,
      login: !this.state.login,
      switch: this.state.login ? "login" : "registration",
    });
  };
          ...
          <div className="box partner">
            <div className="form">
              ...
              <InputGroup
                class="switch"
                type="button"
                text={`Swicth to ${this.state.switch}`}
